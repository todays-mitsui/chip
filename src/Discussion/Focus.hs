...
goAppL (App (t1:t2:[]), bs) =
  case bs of
       (AppRCrumb ts' : bs') -> Just (t1 , AppRCrumb (t2:ts') : bs')
       _                     -> Just (t1 , AppRCrumb [t2]     : bs)
goAppL (App ts        , bs) =
  case bs of
       (AppRCrumb ts' : bs') -> Just (App (init ts), AppRCrumb (last ts:ts') : bs')
       _                     -> Just (App (init ts), AppRCrumb [last ts]     : bs)
goAppL _                    = Nothing
...
goUp (App ts, AppRCrumb (t :ts') : bs) = Just (App (ts ++ [t]), AppRCrumb ts' : bs)
goUp (t1    , AppRCrumb (t2:[])  : bs) = Just (App [t1, t2]   , bs)
goUp (t1    , AppRCrumb (t2:ts') : bs) = Just (App [t1, t2]   , AppRCrumb ts' : bs)
